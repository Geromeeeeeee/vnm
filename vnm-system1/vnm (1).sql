...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 16, 2026 at 11:07 AM
...
(80, 140, NULL, '2026-01-15 00:00:00', '11', 123);
...
(139, 16, 19, 'uploads/licenses/license_6961b1d9650a85.19385916.jpg', '2026-01-10', '22:55:00', 2, 4400.00, NULL, NULL, NULL, 'uploads/payments/proof_139_6961b2293a524.png', 'gcash', '1', NULL, 'Returned', '2026-01-10 09:56:41', 'Proof Uploaded', 'Scheduled'),
(140, 18, 19, 'uploads/licenses/license_69692748a7c3d5.27218951.jpg', '2026-01-17', '13:43:00', 5, 11000.00, NULL, NULL, NULL, 'uploads/payments/proof_140_6969276b8e0e8.jpg', 'gcash', '123456789', NULL, 'Returned', '2026-01-16 01:43:36', 'Proof Uploaded', 'Scheduled');
...
(56, 139, 1, '2026-01-15 18:45:00', '2', 1564, 0.00, 0.00),
(57, 140, 1, '2026-01-19 06:47:00', '0', 1234, 0.00, 0.00);
...
(39, 140, 18, '2026-01-15 00:00:00', 11000.00, 'Returned', '2026-01-23', '01:46:00');

-- --------------------------------------------------------

--
-- Stand-in structure for view `rental_summary`
...
,`rental_duration_days` int(11)
,`payment_status` enum('Unpaid','Paid','Proof Uploaded')
,`request_status` enum('Pending','Approved','Rejected','Cancelled','Picked Up','Returned','Early Return Requested','Early_Return_Approved','Early_Return_Scheduled')
...
,`model` varchar(100)
,`plate_no` varchar(20)
,`final_amount_settled` decimal(12,2)
);
...
(18, 'Gerome Emmanuel', 'ethel.cainta.2024@email.com', '09270277139', 'Imus City', 'ABC 123', '$2y$10$X2Iz7RX2mR688.DdS6SlKez2olD3sqNIg9ue4H0Nmv2n/HApx8OFO', 1, 0);
...
--
-- Structure for view `rental_summary`
--
DROP TABLE IF EXISTS `rental_summary`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `rental_summary`  AS SELECT `rr`.`request_id` AS `request_id`, `rr`.`driver_license_photo` AS `driver_license_photo`, `rr`.`rental_date` AS `rental_date`, `rr`.`rental_time` AS `rental_time`, `rr`.`total_cost` AS `total_cost`, `rr`.`rental_duration_days` AS `rental_duration_days`, `rr`.`payment_status` AS `payment_status`, `rr`.`request_status` AS `request_status`, `rr`.`payment_proof_path` AS `payment_proof_path`, `rr`.`payment_reference_no` AS `payment_reference_no`, `rr`.`admin_notes` AS `admin_notes`, `u`.`fullname` AS `customer_name`, `c`.`car_brand` AS `car_brand`, `c`.`model` AS `model`, `c`.`plate_no` AS `plate_no`, coalesce(`rrr`.`total_deducted_cost`,`rr`.`total_cost` + ifnull(`rd`.`damage_fee`,0) - ifnull(`rd`.`final_refund_amount`,0),`rr`.`total_cost`) AS `final_amount_settled` FROM ((((`rental_requests` `rr` join `users` `u` on(`rr`.`user_id` = `u`.`user_id`)) join `cars` `c` on(`rr`.`car_id` = `c`.`car_id`)) left join `rental_return_requests` `rrr` on(`rr`.`request_id` = `rrr`.`request_id`)) left join `rental_return_details` `rd` on(`rr`.`request_id` = `rd`.`request_id`)) WHERE `rr`.`request_status` = 'Returned' ;
...
--
ALTER TABLE `rental_pickup_details`
  MODIFY `pickup_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=81;

--
-- AUTO_INCREMENT for table `rental_requests`
--
ALTER TABLE `rental_requests`
  MODIFY `request_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=141;
